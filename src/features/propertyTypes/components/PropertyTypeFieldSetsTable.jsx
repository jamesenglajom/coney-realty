				...
				<table className="w-full min-w-[560px] text-left border-collapse">
					...
					<tbody className="divide-y divide-theme-gold-light/70 dark:divide-border-dark">
						{fieldSets.map((fieldSet) => (
							<tr key={fieldSet.id} className="hover:bg-bg-light dark:hover:bg-white/[0.02]">
									...
									{fieldSet.fields.length === 0 ? (
										...
										<div className="flex flex-wrap gap-1.5">
											{fieldSet.fields.map((field) => (
												<Badge
													key={field.key}
													className="bg-theme-gold-light text-theme-blue dark:bg-white/10 dark:text-theme-gold"
												>
													{field.label}
												</Badge>
											))}
