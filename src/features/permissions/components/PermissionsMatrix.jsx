...
export default function PermissionsMatrix({ initialPermissions }) {
	...
	return (
		<div className="space-y-8">
			<div className="rounded-xl border border-theme-gold-light bg-theme-gold-light/40 p-4 text-sm text-txt-secondary dark:border-[#333] dark:bg-white/[0.03] dark:text-txt-secondary-dark">
						...
						<table className="w-full min-w-[420px] text-left border-collapse">
							<thead>
								<tr className="border-b border-theme-gold-light bg-[#fcfcfc] dark:border-[#333] dark:bg-black/40">
									...
									{PERMISSION_ACTIONS.map((action) => (
										<th
											key={action}
											className="p-3 text-center text-xs font-bold uppercase tracking-wider text-txt-muted dark:text-txt-muted-dark"
										>
							...
							<tbody className="divide-y divide-theme-gold-light dark:divide-[#333]">
								{PAGES.map((page) => {
									...
									return (
										<tr key={page}>
											<td className="p-3 text-sm font-medium capitalize text-theme-blue dark:text-white">{page}</td>
											{PERMISSION_ACTIONS.map((action) => (
